...
class NLPUValidator:
    ...
    def load_budget(
        ...
        validator_output_data: bool = False, output_dir_file: str = None
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            df = transform_case(
                read_data(
                    file_path=file_path,
                    sheet_name=self.settings.get("module_validator_nlpu.budget_data.sheet_name", "Tables"),
                ),
        ...
        df = calculate_total_item(
            ...
            column_quantity=self.settings.get("module_validator_nlpu.budget_data.column_quantity", "qtde"),
        ...
        if self.settings.get("module_validator_nlpu.budget_data.columns_to_rename"):
            df = rename_columns(
                df, rename_dict=self.settings.get("module_validator_nlpu.budget_data.columns_to_rename")
            )
        ...
        **kwargs,
        ...
        if detect:
            ...
            if report.format == "wide" and target == "long":
                df_wide, df_long = self.wide_to_long(df, col_to_regiao_grupo=report.columns, **kwargs)
    ...
    def load_lpu(
        ...
        if self.settings.get("module_validator_lpu.lpu_data.columns_to_rename"):
            df_long = rename_columns(
                df_long, rename_dict=self.settings.get("module_validator_lpu.lpu_data.columns_to_rename")
            )
    ...
    def validate_nlpu(
        ...
        try:
            ...
            # Validador se é desejado fazer o merge fuzzy
            ...
            if validator_use_merge_fuzzy_budget_lpu:
                ...
                df_match_fuzzy_budget_lpu = self.apply_match_fuzzy_budget_lpu(
                    df_left=df_budget,
...
if __name__ == "__main__":
    validator = NLPUValidator()
    sys.exit(validator.orchestrate_validate_nlpu())
